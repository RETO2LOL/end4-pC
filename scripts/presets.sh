...
# Usage:
#   presets.sh --save <name> [description] [command]
...
case "$action" in
    --save)
        # Overwrite updates the style while retaining the local preset's metadata.
        if [ -f "$PRESETS_DIR/${name}.json" ]; then
            if [ "${#args[@]}" -lt 2 ]; then
                description=$(jq -r '._presetMeta.description // ""' "$PRESETS_DIR/${name}.json")
            fi
            if [ "${#args[@]}" -lt 3 ]; then
                preset_command=$(jq -r '._presetMeta.command // ""' "$PRESETS_DIR/${name}.json")
            fi
        fi
        jq "$BLACKLIST_FILTER" "$CONFIG_FILE" > "$PRESETS_DIR/${name}.json"
        if [ -n "$description" ] || [ -n "$preset_command" ]; then
            jq --arg desc "$description" --arg cmd "$preset_command" '._presetMeta = {"description": $desc, "command": $cmd}' \
    ...
    --apply)
        ...
        # Commands belong to locally saved presets; online/imported presets only apply styles.
        preset_command=""
        if ! $online && ! $imported; then
            preset_command=$(jq -r '._presetMeta.command // ""' "$preset_file")
        fi
        ...
        if [ -n "$preset_command" ]; then
            bash -c "$preset_command" >/dev/null 2>&1 &
        fi
        ;;
    --export-zip)
        preset_file="$PRESETS_DIR/${name}.json"
